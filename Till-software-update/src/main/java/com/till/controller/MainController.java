...
public class MainController implements Initializable {
    ...
    private void processSuccessfulPayment(double total, double tendered, double change, String paymentMethod) {
        if (!checkAssistanceCalls(total)) return;
        ...
        alert.setContentText(
                "Total: £" + String.format(Locale.UK, "%.2f", total) + "\n" +
                        ("CASH".equals(paymentMethod)
                                ? "Cash Given: £" + String.format(Locale.UK, "%.2f", tendered) +
                                "\nChange: £" + String.format(Locale.UK, "%.2f", change)
                                : "No change required") + "\n\nThank you!"
        );
        alert.showAndWait();
        ...
        salesDAO.logTransaction(new ArrayList<>(cartService.getCartItems()), total, paymentMethod);
        ...
        if (couponField != null) couponField.clear();
        cashField.clear();
    ...
    private void checkLowStock() {
        ...
        for (Product p : allProducts) {
            if (p.getStock() > 0 && p.getStock() <= 5) {
                lowStockMsg.append("• ").append(p.getName())
                        .append(" — only ").append(p.getStock()).append(" left\n");
                auditDAO.logEvent("LOW_STOCK",
                        p.getName() + " stock is low: " + p.getStock(),
                        adminMode ? "ADMIN" : "CASHIER");
            } else if (p.getStock() == 0) {
                lowStockMsg.append("• ").append(p.getName()).append(" — OUT OF STOCK\n");
                auditDAO.logEvent("OUT_OF_STOCK",
        ...
        if (lowStockMsg.length() > 0) {
            ...
            alert.setHeaderText("⚠ The following items need restocking:");
            alert.setContentText(lowStockMsg.toString());
            alert.getDialogPane().setStyle("-fx-border-color: #f39c12; -fx-border-width: 3;");
            alert.showAndWait();
        }
    }
    ...
    private boolean checkAssistanceCalls(double finalTotal) {
        ...
        for (AssistanceCall call : calls) {
            ...
            auditDAO.logEvent("ASSISTANCE_CALL",
                    call.getType() + ": " + call.getReason(),
                    adminMode ? "ADMIN" : "CASHIER");
        }

        boolean managerOverrideRequested = calls.stream()
                .anyMatch(c -> c.getType() == AssistanceType.MANAGER_OVERRIDE);
    ...
    private void handleEndOfDay() {
        ...
        done.setContentText("Report generated. Total sales today: £" +
                String.format(Locale.UK, "%.2f", total));
        done.showAndWait();
    }
    ...
    private void generateEndOfDayPdf(List<SalesRecord> breakdown, double total, int txCount) {
        ...
        try {
            ...
            document.add(new Paragraph("----------------------------------------"));
            document.add(new Paragraph(
                    String.format(Locale.UK, "Total Revenue: £%.2f", total)).setBold());
            document.close();
            Runtime.getRuntime().exec("cmd /c start " + filePath);
        } catch (Exception e) {
    ...
    private void openAnalytics() {
        try {
            ...
            VBox pane = loader.load();
            Stage stage = new Stage();
            stage.setTitle("Sales Analytics Dashboard");
            stage.setScene(new Scene(pane, 1100, 700));
            stage.initOwner(splitPane.getScene().getWindow());
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.showAndWait();
        } catch (IOException e) {
            LOGGER.log(Level.WARNING, "Failed to open analytics dashboard", e);
            new Alert(Alert.AlertType.ERROR, "Failed to open analytics dashboard").showAndWait();
        }
    }
    ...
    private void syncTransactionPayload(TransactionPayload payload) {
        ...
        if (!synced) {
            ...
            auditDAO.logEvent("API_SYNC_QUEUED", "Transaction sync queued for retry",
                    adminMode ? "ADMIN" : "CASHIER");
            LOGGER.warning("Transaction API sync failed; queued for retry");
        } else {
            auditDAO.logEvent("API_SYNC_SUCCESS", "Transaction synced successfully",
                    adminMode ? "ADMIN" : "CASHIER");
        }
    }

    private void processPendingSyncQueue() {
        List<QueuedTransaction> pending = transactionSyncDAO.getPending(
                AppConfig.transactionSyncBatchSize());
        for (QueuedTransaction queuedTransaction : pending) {
            boolean synced = transactionApiClient.submitJson(
                    queuedTransaction.getPayloadJson());
            if (synced) {
                transactionSyncDAO.markSynced(queuedTransaction.getId());
            } else {
                transactionSyncDAO.markFailed(queuedTransaction.getId(),
                        "Sync failed at startup retry");
            }
        }
    }
}