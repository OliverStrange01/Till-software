...
public class MainController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Register the main scene once the window is shown so settings apply immediately
        javafx.application.Platform.runLater(() -> {
            if (splitPane.getScene() != null) {
                SettingsManager.getInstance().register(splitPane.getScene());
            }
        });
    }
    ...
    private void handleBarcodeInput() {
        ...
        String allergens = found.getAllergens();
        if (allergens != null && !allergens.trim().isEmpty()) {
            resultLabel.setText("⚠ ALLERGENS: " + allergens.trim() + " — " + found.getName() + " added.");
            resultLabel.setStyle("-fx-text-fill: #e65100; -fx-font-weight: bold;");
        } else {
    ...
    private void openAdminStock() {
        try {
            ...
            Scene scene = new Scene(adminPane, 900, 600);
            SettingsManager.getInstance().register(scene);
            adminStage.setScene(scene);
            adminStage.initOwner(splitPane.getScene().getWindow());
            adminStage.initModality(Modality.APPLICATION_MODAL);
            adminStage.setOnHidden(e -> SettingsManager.getInstance().unregister(scene));
    ...
    private void openSettings() {
        try {
            ...
            stage.setOnHidden(e -> SettingsManager.getInstance().unregister(scene));
            stage.showAndWait();
        } catch (IOException e) {
            LOGGER.log(Level.WARNING, "Failed to open settings window", e);
            new Alert(Alert.AlertType.ERROR, "Failed to open settings").showAndWait();
        }
    }
    ...
    private void openAnalytics() {
        try {
            ...
            LOGGER.log(Level.WARNING, "Failed to open analytics window", e);
            new Alert(Alert.AlertType.ERROR, "Failed to open analytics").showAndWait();
        }
    }
